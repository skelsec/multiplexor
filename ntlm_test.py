...
# https://support.symantec.com/en_US/article.HOWTO54161.html
# https://support.microsoft.com/en-us/help/823659/client-service-and-program-issues-can-occur-if-you-change-security-set
import io
import os
import enum
import base64
...
class NTLMAuthenticate:
	...
	def from_buffer(buff, _use_NTLMv2 = True):
		...
		#print('NT len: %d' % auth.NtChallengeResponseFields.length)
		#print('LM len: %d' % auth.LmChallengeResponseFields.length)
		#if auth._use_NTLMv2 or auth.NtChallengeResponseFields.length > 24:
		if auth.NtChallengeResponseFields.length > 24:
...
class NTLMAUTHHandler:
	...
	def do_AUTH(self, authData):
		if self.ntlmNegotiate is None:
			...
			return (None, self.ntlmChallenge.to_bytes(), None)
...
class NTLMCredentials:
	...
	def construct(ntlmNegotiate, ntlmChallenge, ntlmAuthenticate):
		...
		#print(ntlmAuthenticate.NtChallengeResponseFields.length)
...
"""
THE FOLLOWING CODE IS FROM https://github.com/benjimin/pywebcorp/blob/master/pywebcorp/ctypes_sspi.py
License is unclear, so be careful.
"""
...
class ClientAuth():
	...
	def authorize(self, challenge=None, flags=65564):	   
		...
		return r, newbuf
"""
Borrowed code part ends here.
"""

class NTLMVersionCheck(enum.Enum):
	NTLMv1 = enum.auto()
	NTLMv1_LM = enum.auto() #this will only yield result on really bad configurations and/or outdated windows versions
	NTLMv1_ESS = enum.auto()
	NTLMv2 = enum.auto()
		
def check_ntlm(version):
	...
	ntlm_handler.setup(setup_params)
	ca = ClientAuth()
...
def check_all_ntlm():
	for version in NTLMVersionCheck:
		print('[+] Checking for %s hashes' % version)
		cred = check_ntlm(version)
		if cred is not None:
			print('[+] Got hashes for version %s' % version)
			print(cred.to_credential().fullhash)
		else:
			print('[-] %s seems to be disabled' % version)
